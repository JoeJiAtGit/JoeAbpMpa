...
namespace JoeAbpMpa.Tests
{
    public abstract class JoeAbpMpaTestBase : AbpIntegratedTestBase<JoeAbpMpaTestModule>
    {
        ...
        protected void UsingDbContext(Action<JoeAbpMpaDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<JoeAbpMpaDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<JoeAbpMpaDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<JoeAbpMpaDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<JoeAbpMpaDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JoeAbpMpaDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<JoeAbpMpaDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JoeAbpMpaDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<JoeAbpMpaDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<JoeAbpMpaDbContext, Task<T>> func)
        {
            T result;
